...
const AdminDashboard = () => {
  ...
  const loadData = async (showSkeleton = true) => {
    ...
    try {
      const [analyticsRes, summaryRes, usersRes] = await Promise.all([
        axios.get('/api/products/analytics/overview'),
        axios.get('/api/products/summary/sales'),
        ...
        <header className="flex flex-col gap-4 rounded-3xl bg-white/80 px-6 py-6 shadow-brand-sm backdrop-blur md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Indicadores clave de los ultimos 30 dias con detalle de ventas, pedidos y clientes.
            </p>
          </div>
