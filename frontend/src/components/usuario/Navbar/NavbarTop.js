...
const NavbarTop = () => {
  ...
  const { branding, settings, loading, isModuleEnabled } = usePublicConfig();
  ...
  const isAdmin = Boolean(user?.isAdmin || user?.role === 'admin' || user?.role === 'superadmin');
  ...
  const brandName = branding?.navbarName || settings?.tradeName || 'Niway Store';
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between py-3 lg:hidden">
              ...
              <Link to="/" className="flex-1 flex items-center justify-center gap-2" aria-label="Volver al inicio">
                ...
                <span className="text-white text-xl font-extrabold uppercase tracking-[0.3em]">{brandName}</span>
              </Link>
            ...
            <div className="hidden lg:flex items-center justify-between py-4 gap-4">
              <Link to="/" className="flex items-center gap-3 flex-shrink-0" aria-label="Volver al inicio">
                ...
                <span className="text-white text-3xl font-extrabold tracking-wide drop-shadow-sm uppercase">{brandName}</span>
              </Link>
            ...
            >
              <div className="py-3 space-y-4">
                {productsEnabled && highlightItems.length > 0 && (
                ...
                {productsEnabled && categoriesEnabled && categoryRows.length > 0 && (
